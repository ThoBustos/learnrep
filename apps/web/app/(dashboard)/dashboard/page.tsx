...
const mockQuizzes: QuizFeedItem[] = [
  { id: '1', title: 'React Server Components', topic: 'React', difficulty: 'medium', questions: 8, attempts: 3, best: 75, author: 'Thomas B.', time: '2h ago' },
  { id: '2', title: 'TypeScript Generics', topic: 'TypeScript', difficulty: 'hard', questions: 10, attempts: 1, best: 60, author: 'Angel C.', time: '4h ago' },
  { id: '3', title: 'Closures & Scope', topic: 'JavaScript', difficulty: 'easy', questions: 6, attempts: 5, best: 90, author: 'Thomas B.', time: 'Yesterday' },
  { id: '4', title: 'SQL Indexes Deep Dive', topic: 'Database', difficulty: 'expert', questions: 12, attempts: 2, best: 45, author: 'John M.', time: 'Yesterday' },
  { id: '5', title: 'Async Patterns in Go', topic: 'Go', difficulty: 'hard', questions: 9, attempts: 0, best: 0, author: 'Angel C.', time: '2d ago' },
]
...
const navItems = [
  ...
  { label: 'My Quizzes', Icon: BookOpen },
        ...
        <main className="flex flex-1 flex-col">
          <header className="flex items-center justify-between border-b-[3px] border-[#151515] bg-[#ffd426] px-5 py-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button type="button" className="flex size-10 items-center justify-center rounded-full border-[3px] border-[#151515] bg-white font-black shadow-[3px_3px_0_#151515] transition-transform hover:-translate-y-0.5">
                +
              </button>
            </div>
          </header>
          ...
          <div className="flex flex-1 flex-col gap-6 p-5 lg:flex-row lg:p-8">
            <div className="flex flex-1 flex-col gap-5">
              ...
              <div className="flex flex-col gap-3 rounded-[1.5rem] border-[3px] border-[#151515] bg-white/70 p-4 shadow-[6px_6px_0_#151515]">
                <div className="flex items-center justify-between">
                  ...
                  <Tabs tabs={['Feed', 'Trending', 'New']} active={activeSection} onChange={setActiveSection} />
                </div>
            ...
            <div className="flex w-full flex-col gap-5 lg:w-72 lg:shrink-0">
              <button type="button" className="w-full rounded-[1.3rem] border-[3px] border-[#151515] bg-[#151515] px-5 py-4 text-left text-[#ffd426] shadow-[5px_5px_0_#ff5858] transition-transform hover:-translate-y-0.5">
                ...
                <p className="mt-2 font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-[#ffd426]/60">Cmd + G to generate</p>
              </button>
        ...
        {navItems.map(({ label, Icon }) => (
          <button
            ...
            className={cn(
              ...
              activeSection === label ? 'bg-[#151515] text-[#ffd426]' : 'text-[#151515]'
            )}
          >
            <Icon className="size-5" />
            {label}
...
function FeedRow({ quiz }: { quiz: QuizFeedItem }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-[1rem] border-[3px] border-[#151515] bg-white p-3 shadow-[3px_3px_0_#151515] transition-transform hover:-translate-y-0.5">
      <div className={cn('flex size-10 shrink-0 items-center justify-center rounded-[0.7rem] border-[3px]', tone.border, tone.bg)}>
        <span className={cn('font-mono text-[9px] font-black uppercase', tone.text)}>{quiz.topic.slice(0, 2)}</span>
      </div>
      <div className="min-w-0 flex-1">
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        <button type="button" className="rounded-[0.7rem] border-[3px] border-[#151515] bg-[#ffd426] px-3 py-1.5 text-xs font-black shadow-[2px_2px_0_#151515] transition-transform hover:-translate-y-0.5">
          Go
        </button>
      </div>
    </div>
...
function LeaderRow({ rank, name, handle, score, streak }: { rank: number; name: string; handle: string; score: number; streak: number }) {
  return (
    <div className={cn(
      ...
      <div className={cn('flex size-8 shrink-0 items-center justify-center rounded-full border-[3px] border-[#151515] font-black text-sm', rank === 1 ? 'bg-[#151515] text-[#ffd426]' : 'bg-[#f5f4f0]')}>
        {rank}
      </div>
      <div className="min-w-0 flex-1">
