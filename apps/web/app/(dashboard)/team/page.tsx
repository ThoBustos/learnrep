...
function normalizeInviteCode(input: string): string {
  ...
  return parts.length > 1 ? parts.at(-1) ?? '' : trimmed
}
  ...
  return (
    <DashboardCanvas>
      ...
      {isLoading ? (
        ...
        <TeamView team={team} feed={feed} feedError={feedError} leaderboard={leaderboard} codeCopied={codeCopied} onCopyInviteCode={copyInviteCode} />
      ) : (
        <NoTeamView
  ...
  return (
    <FormGrid>
      <SetupCard
        icon={Users}
        title="Create a team"
        description="Start a team and invite teammates with a code."
      >
        <TextInput
      ...
      <SetupCard
        iconLabel="#"
        title="Join a team"
        description="Paste an invite code or URL from a teammate."
      >
        <TextInput
