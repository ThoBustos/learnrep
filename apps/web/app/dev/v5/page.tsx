...
// v5 — "Dedicated Agent Section"
// A standalone dark section between hero and features.
// Each agent name is a pill. Subtext explains why it works with any harness.
...
const AGENTS = [
  ...
  { name: 'Codex CLI', maker: 'OpenAI' },
  { name: 'Amp', maker: 'Sourcegraph' },
  { name: 'Gemini CLI', maker: 'Google' },
  { name: 'Copilot CLI', maker: 'Microsoft' },
  { name: 'Aider', maker: 'Open source' },
  { name: 'Goose', maker: 'Block' },
  { name: 'Cline', maker: 'Open source' },
  { name: 'OpenCode', maker: 'Open source' },
  { name: 'OpenHands', maker: 'All-Hands AI' },
  { name: 'Open Interpreter', maker: 'Open source' },
  { name: 'Warp', maker: 'Warp' },
]
...
const QUESTIONS: QuestionDef[] = [
  ...
  },
  {
    ...
    options: ['git log --lines', 'git diff --author', 'git blame', 'git show --per-line'],
    correctIndex: 2,
    ...
    <div className="min-h-screen bg-[#fafaf8] text-[#151515]" style={{ fontFamily: 'var(--font-space-grotesk)' }}>
      ...
      <header className="sticky top-0 z-20 flex items-center justify-between border-b border-[#151515]/10 bg-[#fafaf8]/90 px-6 py-4 backdrop-blur-sm sm:px-10">
        ...
        <nav className="flex items-center gap-5">
          <Link href="/docs" className="hidden items-center gap-1.5 rounded-[0.9rem] border-[3px] border-[#151515] bg-white px-3 py-2 font-mono text-[10px] font-black shadow-[2px_2px_0_#151515] transition-transform hover:-translate-y-0.5 sm:flex">
            <BookOpen className="size-3" />
            Docs
          </Link>
          <div className="hidden sm:flex"><GitHubStarButton stars={stars} /></div>
          <Link href="/login" className="rounded-[0.6rem] border-[2.5px] border-[#151515] bg-[#ffd426] px-4 py-1.5 font-mono text-[11px] font-black uppercase tracking-widest text-[#151515] shadow-[3px_3px_0_#151515] transition-transform hover:-translate-y-0.5">
            Sign in
          </Link>
        </nav>
      </header>
      ...
      <section className="mx-auto grid max-w-6xl grid-cols-1 gap-10 px-6 py-16 sm:px-10 lg:grid-cols-2 lg:items-center lg:gap-16 lg:py-24">
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="group relative overflow-hidden rounded-[0.9rem] bg-[#151515] shadow-[0_8px_24px_rgba(21,21,21,0.18)]">
              ...
              <button type="button" onClick={copyInstall} className="absolute right-2 top-1/2 -translate-y-1/2 flex size-7 items-center justify-center rounded-[0.5rem] border-[2px] border-[#ffd426]/40 bg-white/10 text-[#ffd426] transition-all hover:border-[#ffd426]/70 hover:bg-white/20">
                {installCopied ? <Check className="size-3.5 text-[#d9ff69]" /> : <Copy className="size-3.5" />}
              </button>
            </div>
            <Link href="/login" className="font-mono text-sm font-bold text-[#151515]/50 hover:text-[#151515]">or sign in</Link>
          </div>
          ...
          <div className="flex items-center gap-3 pt-1">
            ...
            <p className="font-mono text-xs font-bold text-[#151515]/50">Engineers quizzing their teams from the terminal</p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="rounded-[1.3rem] border-[3px] border-[#151515] bg-[#ffd426] p-4 shadow-[5px_5px_0_#151515]">
            <p className="font-mono text-[10px] font-black uppercase tracking-widest text-[#151515]/60">Then in your browser</p>
      ...
      {/* ── AGENT COMPATIBILITY (new section) ────────────────────────────────── */}
      <section className="border-t-[3px] border-[#151515] bg-[#151515] px-6 py-14 sm:px-10">
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-col items-center gap-8 text-center">
            <div>
              <p className="font-mono text-[11px] font-black uppercase tracking-[0.25em] text-white/40">
                Works inside any AI agent
              </p>
              <p className="mt-3 max-w-lg text-base font-bold text-white/60">
                lr runs as a subprocess. Any agent that can run shell commands can use it.
              </p>
            </div>
            <div className="flex flex-wrap justify-center gap-2">
              {AGENTS.map(({ name, maker }) => (
                <div
                  key={name}
                  className="flex flex-col items-center rounded-[0.9rem] border-[2px] border-white/10 bg-white/5 px-4 py-3"
                >
                  <span className="font-mono text-sm font-black text-white">{name}</span>
                  <span className="mt-0.5 font-mono text-[10px] font-bold text-white/30">{maker}</span>
                </div>
              ))}
            </div>
        ...
        {!done ? (
          <>
            <div className="mb-8 text-center">
              <p className="font-mono text-[11px] font-black uppercase tracking-[0.25em] text-[#151515]/50">No account needed</p>
              <h2 className="mt-2 text-4xl font-black tracking-[-0.04em] sm:text-5xl">See what your<br />team will take.</h2>
              <p className="mt-3 font-mono text-sm font-bold text-[#151515]/60">Three questions. This is what lr generate produces.</p>
            </div>
            <div className="mb-6">
              <div className="h-1.5 w-full overflow-hidden rounded-full bg-[#151515]/20">
                <div className="h-full rounded-full bg-[#151515] transition-all duration-500" style={{ width: `${progress}%` }} />
              </div>
              <p className="mt-2 font-mono text-[10px] font-bold uppercase tracking-widest text-[#151515]/50">Q{questionIndex + 1} of {total}</p>
            </div>
            ...
            {phase === 'feedback' && (
              <div className={cn('mt-4 rounded-[1rem] border-[3px] border-[#151515] p-4 shadow-[4px_4px_0_#151515]', selectedOption === question.correctIndex ? 'bg-[#d9ff69]' : 'bg-[#ff6b62]')}>
            ...
            <div className="mt-4 flex flex-col gap-2.5">
              {question.options.map((option, i) => {
                ...
                return (
                  <button key={i} type="button" disabled={phase !== 'question'} onClick={() => setSelectedOption(i)}
                    className={cn('flex items-center gap-4 rounded-[1rem] border-[3px] p-4 text-left shadow-[3px_3px_0_#151515] transition-all', cardStyle, phase === 'question' && 'hover:-translate-y-0.5 hover:shadow-[4px_4px_0_#151515]', phase !== 'question' && 'cursor-default')}>
                    <span className={cn('flex size-8 shrink-0 items-center justify-center rounded-full border-[3px] border-[#151515] font-mono text-xs font-black', letterStyle)}>{LETTERS[i]}</span>
            ...
            <div className="mt-4">
              {phase === 'question' ? (
                <button type="button" onClick={handleSubmit} disabled={selectedOption === null}
                  className={cn('w-full rounded-[1rem] border-[3px] border-[#151515] py-4 text-lg font-black shadow-[4px_4px_0_#151515] transition-all', selectedOption !== null ? 'bg-[#151515] text-[#ffd426] hover:-translate-y-0.5' : 'cursor-not-allowed bg-[#151515]/20 text-[#151515]/40')}>
                  Submit Answer
                </button>
              ) : isLast ? (
                <button type="button" onClick={handleNext} className="w-full rounded-[1rem] border-[3px] border-[#151515] bg-[#151515] py-4 text-lg font-black text-[#ffd426] shadow-[4px_4px_0_#ff5858] transition-transform hover:-translate-y-0.5">
                  See Results
                </button>
              ) : (
                <button type="button" onClick={handleNext} className="w-full rounded-[1rem] border-[3px] border-[#151515] bg-[#ffd426] py-4 text-lg font-black shadow-[4px_4px_0_#151515] transition-transform hover:-translate-y-0.5">
          ...
          <div className="flex flex-col items-center gap-6">
            <div className={cn('flex w-full flex-col items-center gap-3 rounded-[1.5rem] border-[3px] border-[#151515] py-10 shadow-[7px_7px_0_#151515]', isHigh ? 'bg-[#d9ff69]' : 'bg-white/90')}>
              {isHigh && <span className="rounded-full border-[2.5px] border-[#1e6f38] bg-[#151515] px-4 py-1 font-mono text-[11px] font-black uppercase tracking-widest text-[#d9ff69]">Nice work!</span>}
              <p className={cn('text-[5rem] font-black leading-none tracking-[-0.06em]', isHigh ? 'text-[#1e6f38]' : 'text-[#151515]')}>{pct}%</p>
              <p className={cn('text-base font-black', isHigh ? 'text-[#1e6f38]' : 'text-[#67606a]')}>{correctCount} of {total} correct</p>
            </div>
            <div className="w-full rounded-[1.3rem] border-[3px] border-[#151515] bg-white/90 p-6 shadow-[5px_5px_0_#151515]">
              ...
              <p className="mt-1 font-mono text-sm font-bold text-[#151515]/60">Generate your own on any topic in one command.</p>
              <div className="mt-4 flex overflow-hidden rounded-[0.9rem] border-[3px] border-[#151515] shadow-[3px_3px_0_#151515]">
                <div className="flex-1 bg-[#151515] px-4 py-3"><code className="font-mono text-sm font-bold text-[#ffd426]">{INSTALL_CMD}</code></div>
                <button type="button" onClick={copyCLI} className="flex items-center gap-1.5 border-l-[3px] border-[#151515] bg-white px-4 py-3 font-mono text-[10px] font-black uppercase tracking-widest transition-colors hover:bg-[#ffd426]">
                  {copied ? <Check className="size-3.5 text-[#1e6f38]" /> : <Copy className="size-3.5" />}
                </button>
              </div>
              <p className="mt-3 font-mono text-[10px] font-bold text-[#151515]/40">Then: <code>lr generate &quot;your topic&quot;</code></p>
            </div>
            <Link href="/login" className="w-full rounded-[1rem] border-[3px] border-[#151515] bg-[#151515] py-4 text-center text-lg font-black text-[#ffd426] shadow-[4px_4px_0_#ff5858] transition-transform hover:-translate-y-0.5">
              Create account <ArrowRight className="inline size-5" />
            </Link>
            <Link href="/docs" className="font-mono text-[11px] font-bold uppercase tracking-widest text-[#151515]/50 hover:text-[#151515]">Read the docs</Link>
          </div>
        )}
      </div>
    </section>
